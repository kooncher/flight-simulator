import { SLOT_START_HOURS } from '../store/booking'

function pad(n: number) {
  return n.toString().padStart(2, '0')
}

export function buildICS(summary: string, date: string, slot: number, durationHours = 1, location = 'Flight Reserve') {
  ...
  const range = `${pad(startH)}:00-${pad(endH)}:00`
  const content = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
